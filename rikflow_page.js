document.rikflow_page = function rikflow_page() {
  ...
  var xlogo = [-3, -2, -1, 0, 1, 2, 3].map(function (d, i) {
    return d * kern + xmid + kshift[i] * size - 0.5 * x1 * size;
  });
  var xlogo2 = [0, 1, 2, 3, 4, 5, 6].map(function (d, i) {
    return d * size * logoScale2 + kshift2[i] * size * logoScale2 + size * logoScale2;
  });
  ...
  z.call(
    function () {
      z.add_linear('x', xlogo[2], (1 - viz.xdelay) * viz.spinDur);
    },
